...
public class GirdThreeAdapter extends BaseAdapter{

    private EncyclopediaBean bean;
    private Context context;

    public GirdThreeAdapter(Context context) {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        myViewHolder.tv.setText(bean.getGroup().get(2).getCategories().get(i).getName());
        Picasso.with(context).load(bean.getGroup().get(2).getCategories().get(i).getImage_url()).into(myViewHolder.img);
        return view;
    }
    class MyViewHolder{
