...
public class GirdOneAdapter extends BaseAdapter{

    private EncyclopediaBean bean;
    private Context context;

    public GirdOneAdapter(Context context) {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        myViewHolder.tv.setText(bean.getGroup().get(0).getCategories().get(i).getName());
        Picasso.with(context).load(bean.getGroup().get(0).getCategories().get(i).getImage_url()).into(myViewHolder.img);
        return view;
    }
    class MyViewHolder{
